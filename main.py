...
winwidth = 1080
winheight = 600
pygame.display.set_caption('Super Minigolf by Akash')
...
def onGreen():  # Determine if we are on the green
    ...
    for i in objects:
        if i[4] == 'green':
            if ballStationary[1] < i[1] + i[3] and ballStationary[1] > i[1] - 20 and ballStationary[0] > \
                    i[0] and ballStationary[0] < i[0] + i[2]:
                return True
            else:
                return False
                ...
                if pos[0] < x[0] + x[2] and pos[0] > x[0] - x[2] and pos[1] < x[1] + x[2] and pos[1] > x[1] - x[2]:
                    ...
                    if powerUps == 0:
                        ...
                        if stickyPower is False and superPower is False and powerUps > 0:
                            ...
                            powerUps -= 1
                            ballColor = (255, 0, 255)
                    elif x[3] == 'M':  # Mullagain, allows you to retry your sot from your previous position,
                        # will remove strokes u had on last shot
        ...
        if not(overHole(ballStationary[0], ballStationary[1])):  # If we aren't in the hole
            ...
            for i in objects:  # for every object in the level
                if i[4] == 'coin':  # If the ball hits a coin
                    if i[5]:
                        if ballCords[0] < i[0] + i[2] and ballCords[0] > i[0] and ballCords[1] > \
                                i[1] and ballCords[1] < i[1] + i[3]:
                            courses.coinHit(level - 1)
                            coins += 1

                if i[4] == 'laser':  # if the ball hits the laser hazard
                    if ballCords[0] > i[0] and ballCords[0] < i[0] + i[2] and ballCords[1] > i[1] \
                            and ballCords[1] < i[1] + i[3]:
                        ballCords = shootPos
                        hazard = True
                        subtract = 0
                    ...
                    if ballCords[1] > i[1] - 6 and ballCords[1] < i[1] + 8 and ballCords[0] < i[0] + i[2]\
                            and ballCords[0] > i[0] + 2:
                        ballCords = shootPos
                        subtract = 0
                        hazard = True
                        ballStationary = ballCords
                    ...
                    if ballCords[1] > i[1] - 2 and ballCords[1] < i[1] + 7 and \
                            ballCords[0] < i[0] + i[2] and ballCords[0] > i[0]:
                        hitting = False
                    ...
                    elif ballCords[1] < i[1] + i[3] and ballCords[1] > i[1] and ballCords[0] > i[0] - 2\
                            and ballCords[0] < i[0] + 10:
                        hitting = False
                        power = physics.findPower(power, angle, time)
                        if angle < math.pi / 2:
                            if not(time > maxT):
                    ...
                    elif ballCords[1] > i[1] + i[3] and ballCords[0] + 2 > i[0] and ballCords[1] <\
                            i[1] + i[3] + 10 and ballCords[0] < i[0] + i[2] + 2:
                        power = physics.findPower(power, angle, time)
                        if not(hitting):
                            hitting = True
                            if angle > math.pi / 2:
